...
class DensePosePredictor(nn.Module):
    def __init__(self, cfg, input_channels):
        ...
        pool_hidden_dim = cfg.MODEL.ROI_DENSEPOSE_HEAD.CONV_HEAD_DIM
        pool_resolution = cfg.MODEL.ROI_DENSEPOSE_HEAD.POOLER_RESOLUTION
        self.dimension_flag = cfg.MODEL.ROI_DENSEPOSE_HEAD.DIMENSION_ON
        ...
        self.lx_lowres = ConvTranspose2d(
            dim_in, dim_out_patches, kernel_size, stride=2, padding=int(kernel_size / 2 - 1)
        )
        self.ly_lowres = ConvTranspose2d(
            dim_in, dim_out_patches, kernel_size, stride=2, padding=int(kernel_size / 2 - 1)
        )
        self.lz_lowres = ConvTranspose2d(
            dim_in, dim_out_patches, kernel_size, stride=2, padding=int(kernel_size / 2 - 1)
        )
        # 3D Dimension
        
        if self.dimension_flag:
            self.conv = nn.Conv2d(dim_in, 3, kernel_size=1, stride=1)
            self.dimension_lowers = nn.Sequential(
                # nn.BatchNorm2d(3),
                nn.Linear(pool_resolution * pool_resolution * 3, 1024),
                nn.Tanh(),
                nn.Linear(1024, 1024),
                nn.Tanh(),
                nn.Linear(1024, 3),
    ...
    def forward(self, head_outputs):
        ...
        lx_lowres = self.lx_lowres(head_outputs)
        ly_lowres = self.ly_lowres(head_outputs)
        lz_lowres = self.lz_lowres(head_outputs)
        # print(ann_index_lowres.shape)
        # print(index_uv_lowres.shape)
        # print(lx_lowres.shape)
        # print(ly_lowres.shape)
        # print(lz_lowres.shape)
        
        if self.dimension_flag:
            # print(head_outputs.flatten(start_dim=1).shape)
            feature = F.relu(self.conv((head_outputs)))
            dimension = self.dimension_lowers(feature.flatten(start_dim=1))
            # print(dimension_lowers.shape)
        ...
        lx = interp2d(lx_lowres)
        ly = interp2d(ly_lowres)
        lz = interp2d(lz_lowres)

        # print(ann_index.shape)
        # print(index_uv.shape)
        # print(lx.shape)
        # print(ly.shape)
        # print(lz.shape)
        ...
        if self.dimension_flag:
            ...
            return (
                (ann_index, index_uv, lx, ly, lz, dimension),
                (ann_index_lowres, index_uv_lowres, lx_lowres, ly_lowres, lz_lowres, dimension),
            )

        return (
            (ann_index, index_uv, lx, ly, lz),
            (ann_index_lowres, index_uv_lowres, lx_lowres, ly_lowres, lz_lowres),
        )
...
class DensePoseDataFilter(object):
    ...
    def __call__(self, proposals_with_targets):
        ...
        for proposals_per_image in proposals_with_targets:
            ...
            # for i, dp_target in enumerate(gt_densepose):
            #     print(dp_target)
            selected_indices = [
...
def densepose_inference(densepose_outputs, detections):
    ...
    print(len(densepose_outputs))
    if len(densepose_outputs) == 6:
        s, index_uv, lx, ly, lz, dimension = densepose_outputs
    else:
        s, index_uv, lx, ly, lz = densepose_outputs
        dimension = torch.zeros(lx.shape[0], 3)
    k = 0
    for detection in detections:
        ...
        ly_i = ly[k : k + n_i]
        lz_i = lz[k : k + n_i]
        dimension_i = dimension[k : k + n_i]
        densepose_output_i = DensePoseOutput(s_i, index_uv_i, lx_i, ly_i, lz_i, dimension_i)
...
def _extract_single_tensors_from_matches_one_image(
    ...
    # print(n_i)
    # print(boxes_xywh_est.tensor.shape)
    # print(boxes_xywh_gt.tensor.shape)
    ...
    if n_i:
        ...
        if hasattr(proposals_targets, "gt_densepose"):
            ...
            for k, box_xywh_est, box_xywh_gt, dp_gt in zip(
                ...
                # print(dp_gt.i.shape)
                # print(dp_gt.x.shape)
                # print(dp_gt.lx.shape)
                # print(dp_gt.segm.unsqueeze(0).shape)
                
                if (dp_gt is not None) and (len(dp_gt.x) > 0):
                    i_gt_all.append(dp_gt.i)
                    x_norm_all.append(dp_gt.x)
                    y_norm_all.append(dp_gt.y)
                    lx_gt_all.append(dp_gt.lx)
                    ly_gt_all.append(dp_gt.ly)
                    lz_gt_all.append(dp_gt.lz)
                    dimension_gt_all.append(dp_gt.dimension.view(-1, 3))
...
def _extract_single_tensors_from_matches(proposals_with_targets):
    ...
    for i, proposals_targets_per_image in enumerate(proposals_with_targets):
        ...
        if not n_i:
            continue
        i_gt_img, x_norm_img, y_norm_img, lx_gt_img, ly_gt_img, lz_gt_img, dimension_gt_img, s_gt_img, bbox_xywh_gt_img, bbox_xywh_est_img, i_bbox_img, i_with_dp_img = _extract_single_tensors_from_matches_one_image(  # noqa
        ...
        lx_gt_all.extend(lx_gt_img)
        ly_gt_all.extend(ly_gt_img)
        lz_gt_all.extend(lz_gt_img)
        dimension_gt_all.extend(dimension_gt_img)
    ...
    if (n > 0) and (len(i_with_dp_all) > 0):
        ...
        lx_gt = torch.cat(lx_gt_all, 0)
        ly_gt = torch.cat(ly_gt_all, 0)
        lz_gt = torch.cat(lz_gt_all, 0)
        dimension_gt = torch.cat(dimension_gt_all, 0)
...
class DensePoseLosses(object):
    def __init__(self, cfg):
        ...
        self.dimension_f  = cfg.MODEL.ROI_DENSEPOSE_HEAD.DIMENSION_ON
        self.d_weight     = cfg.MODEL.ROI_DENSEPOSE_HEAD.DIMENSION_WEIGHTS
        # fmt: on
    ...
    def __call__(self, proposals_with_gt, densepose_outputs):
        ...
        if self.dimension_f:
            s, index_uv, lx, ly, lz, d = densepose_outputs
        else:
            s, index_uv, lx, ly, lz = densepose_outputs

        assert lx.size(2) == ly.size(2)
        assert lx.size(3) == lz.size(3)
        assert lx.size(2) == index_uv.size(2)
        assert lx.size(3) == index_uv.size(3)
        # print(lx)
        with torch.no_grad():
            index_uv_img, i_with_dp, bbox_xywh_est, bbox_xywh_gt, index_gt_all, x_norm, y_norm, lx_gt_all, ly_gt_all, lz_gt_all, dimension_gt_all, s_gt, index_bbox = _extract_single_tensors_from_matches(  # noqa
                proposals_with_gt
            )
        n_batch = len(i_with_dp)
        # print(bbox_xywh_gt.shape)
        # print(n_batch)
        # print(dimension_gt.shape)
        ...
        if not n_batch:
            ...
            losses["loss_densepose_lx"] = lx.sum() * 0
            losses["loss_densepose_ly"] = ly.sum() * 0
            losses["loss_densepose_lz"] = lz.sum() * 0
            losses["loss_densepose_I"] = index_uv.sum() * 0
            losses["loss_densepose_S"] = s.sum() * 0
            if self.dimension_f:
                losses["loss_densepose_D"] = d.sum() * 0
            return losses
        ...
        lx_gt = lx_gt_all[j_valid_fg]
        lx_est_all = _extract_at_points_packed(
            lx[i_with_dp],
            index_bbox,
            index_gt_all,
            y_lo,
        ...
        lx_est = lx_est_all[j_valid_fg]

        ly_gt = ly_gt_all[j_valid_fg]
        ly_est_all = _extract_at_points_packed(
            ly[i_with_dp],
            index_bbox,
            index_gt_all,
            y_lo,
        ...
        ly_est = ly_est_all[j_valid_fg]

        lz_gt = lz_gt_all[j_valid_fg]
        lz_est_all = _extract_at_points_packed(
            lz[i_with_dp],
            index_bbox,
            index_gt_all,
            y_lo,
        ...
        # print(F.smooth_l1_loss(lx_est, lx_gt, reduction="sum"))
        # print(lx_gt)
        # print(torch.sum(lx_est), torch.sum(lx_gt))
       
        lx_loss = F.smooth_l1_loss(lx_est, lx_gt, reduction="sum") * self.w_points
        losses["loss_densepose_lx"] = lx_loss
        ly_loss = F.smooth_l1_loss(ly_est, ly_gt, reduction="sum") * self.w_points
        losses["loss_densepose_ly"] = ly_loss
        lz_loss = F.smooth_l1_loss(lz_est, lz_gt, reduction="sum") * self.w_points
        losses["loss_densepose_lz"] = lz_loss
        ...
        if self.dimension_f:
            # print(torch.sum(dimension_gt_all))
            losses["loss_densepose_D"] = F.smooth_l1_loss(dimension_gt_all, d, reduction='sum') * self.d_weight
        else:
            losses["loss_densepose_D"] = 0
        return losses
