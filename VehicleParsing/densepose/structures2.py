...
class DensePoseOutput(object):
    def __init__(self, S, I, LX, LY, LZ):
        self.S = S
        self.I = I  # noqa: E741
        self.LX = LX
        self.LY = LY
        self.LZ = LZ
        self._check_output_dims(S, I, LX, LY, LZ)

    def _check_output_dims(self, S, I, LX, LY, LZ):
        assert (
            len(S.size()) == 4
    ...
    def to_result(self, boxes_xywh):
        ...
        """
        result = DensePoseResult(boxes_xywh, self.S, self.I, self.LX, self.LY, self.LZ)
        return result
    ...
    def __str__(self):
        s = "DensePoseOutput S {}, I {}, LX {}, LY {}, LZ {}".format(
            list(self.S.size()), list(self.I.size()), list(self.LX.size()), list(self.LY.size()), list(self.LZ.size())
        )
        return s
...
class DensePoseResult(object):
    def __init__(self, boxes_xywh, S, I, LX, LY, LZ):
        self.results = []
        ...
        for i, box_xywh in enumerate(boxes_xywh):
            # print(LX[[i]])
            result_i = self._output_to_result(box_xywh, S[[i]], I[[i]], LX[[i]], LY[[i]], LZ[[i]])
    ...
    def _output_to_result(self, box_xywh, S, I, LX, LY, LZ):
        x, y, w, h = box_xywh
        w = max(int(w), 1)
        h = max(int(h), 1)
        result = torch.zeros([4, h, w], dtype=torch.float32, device=LX.device)
        assert (
            len(S.size()) == 4
