...
def densepose_inference(densepose_outputs, detections):
    ...
    for detection in detections:
        ...
        ly_i = ly[k : k + n_i]
        lz_i = lz[k : k + n_i]
        densepose_output_i = DensePoseOutput(s_i, index_uv_i, lx_i, ly_i, lz_i)
...
def _extract_single_tensors_from_matches(proposals_with_targets):
    ...
    for i, proposals_targets_per_image in enumerate(proposals_with_targets):
        ...
        if not n_i:
            continue
        i_gt_img, x_norm_img, y_norm_img, lx_gt_img, ly_gt_img, lz_gt_img, s_gt_img, bbox_xywh_gt_img, bbox_xywh_est_img, i_bbox_img, i_with_dp_img = _extract_single_tensors_from_matches_one_image(  # noqa
...
class DensePoseLosses(object):
    ...
    def __call__(self, proposals_with_gt, densepose_outputs):
        ...
        # print(lx)
        with torch.no_grad():
            index_uv_img, i_with_dp, bbox_xywh_est, bbox_xywh_gt, index_gt_all, x_norm, y_norm, lx_gt_all, ly_gt_all, lz_gt_all, s_gt, index_bbox = _extract_single_tensors_from_matches(  # noqa
                proposals_with_gt
            )
